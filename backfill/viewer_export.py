...
def parse_dsr_frames(dsr_dir: Path) -> dict[tuple[int, int], Path]:
    """Map (channel, timepoint) -> DSR frame path.

    Only `_C<c>_T<t>.tif` frames are considered, which excludes PetaKit5D's
    own `MIPs/` output and any `*_MIP_z.tif` sitting beside the frames.
    """
    frames: dict[tuple[int, int], Path] = {}
    for p in sorted(Path(dsr_dir).glob("*.tif")):
...
def write_ome_zarr(
    ...
    *,
    ...
    frames = parse_dsr_frames(dsr_dir)
    if not frames:
        raise FileNotFoundError(f"no _C<c>_T<t>.tif frames under {dsr_dir}")
...
def write_chimerax_script(dsr_dir: Path, out_path: Path, *, voxel_um: float = DSR_VOXEL_UM) -> Path:
    ...
    files as separate models otherwise.
    """
    frames = parse_dsr_frames(Path(dsr_dir))
    channels = sorted({c for c, _ in frames})
    lines = [
...
def export_for_viewers(
    ...
    *,
    ...
    """Stamp the DSR frames for ChimeraX and build the OME-Zarr for napari."""
    ...
    cxc_path = write_chimerax_script(dsr_dir, out_dir / f"{name}_dsr.cxc", voxel_um=voxel_um)
    summary = {"frames": len(frames), "stamped": stamped,
               "ome_zarr": str(zarr_path), "chimerax": str(cxc_path),
               "voxel_um": voxel_um}
